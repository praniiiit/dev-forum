...
export default function FavComments(props) {
  ...
  const updateComments = () => {
    axios.get('http://localhost:8000/threads').then(response => {
      ...
      for (let thread of response.data) {
        const eachThreadComments = thread.comments.filter(comment => comment.fav);
        eachThreadComments.length && favComments.push(eachThreadComments);
      }
      // console.log(favComments);
      setComments(favComments);
    })
  }
  useEffect(() => {
    updateComments();
  ...
  return (
    ...
    >
      <CardHeader
        title='Favourite Comments'
        subheader={'Comments added to favourites by ' + props.user}
      />
